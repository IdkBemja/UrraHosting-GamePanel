...
#
# Group is `gamedata`, not `game`: this directory is also bind-mounted into
# the dashboard container (a different uid entirely, see dashboard/
# Dockerfile), which needs to both read software the game process wrote AND
# delete it on a clean reinstall (services/installer.py's wipe step used to
# fail with EACCES on exactly this - files this process leaves behind with
# no group access for the dashboard's uid). This pass is only a one-time
# catch-up for whatever already exists right now, at container start - it
# does NOT reliably cover files the game process creates later, at runtime
# (e.g. Minecraft's bundler self-extracting libraries/ and versions/ on
# first boot, well after this line runs): the setgid bit below was
# originally meant to make those inherit the gamedata group automatically,
# but that propagation to new subdirectories turned out to be unreliable on
# Docker Desktop's bind-mount backend, so this repo's Dockerfile now makes
# `gamedata` the game user's PRIMARY group instead - new files get the
# right group from process defaults alone, independent of setgid. `umask
# 002` below (not the default 022) is what then keeps them group-writable
# too, since a directory's own permission bits are never inherited by
# files created inside it, setgid or not.
chown -R "${RUNTIME_UID}:gamedata" "$DATA_DIR" || log "advertencia: no se pudo aplicar chown a $DATA_DIR (normal en bind mounts de Docker Desktop en Windows); continuando"
chmod -R g+rwX "$DATA_DIR" || log "advertencia: no se pudo aplicar chmod a $DATA_DIR; continuando"
find "$DATA_DIR" -type d -exec chmod g+s {} + 2>/dev/null || log "advertencia: no se pudo aplicar el bit setgid en $DATA_DIR; continuando"
umask 002
